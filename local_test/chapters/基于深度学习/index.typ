...
第 3 章的分析表明，多光谱波段与可见光特征能够区分脱叶率与吐絮率等级，但两项任务对信息源的需求不同：脱叶率主要依赖多光谱波段信息，吐絮率主要依赖可见光颜色，人工统计量对冠层纹理与空间结构的描述也较为有限。在生产中，多数作业无人机仅配备可见光相机，且脱叶率与吐絮率需要在同一次作业中同时判读。因此，本章面向仅有可见光影像的条件，研究轻量级双指标联合识别方法。深度卷积网络能够直接从数据中学习层次化的图像特征，在农业表型识别任务上已展现出明显优势@lecun2015deep@kamilaris2018deep；然而主流深度模型参数量与计算量大，而棉花脱叶催熟作业的检测需求最终要落到机载或田间端侧设备上，模型必须在精度与推理开销之间取得平衡@howard2017mobilenets@sandler2018mobilenetv2。此外，脱叶率与吐絮率是同一次催熟作业下相伴发生的两个指标，共享冠层颜色与纹理线索，若分别训练两个独立模型，会重复提取相似特征，增加端侧存储与计算开销。

基于上述考虑，本章设计轻量级双指标识别模型 RTCMNet：以多尺度卷积注意力模块增强网络对叶片脱落区域与棉絮区域的特征表达@hu2018senet@woo2018cbam，并以共享主干加分任务输出的结构同时预测脱叶率与吐絮率等级，在控制参数量与计算量的前提下提升识别精度。本章通过骨干网络对比、分任务评价、消融实验和设备推理测试检验模型表现，研究工作的整体流程如@fig:workflow 所示。

本章其余内容安排如下：5.2 节介绍 RTCMNet 的网络结构与多尺度卷积注意力设计；5.3 节说明训练与评价协议，并报告模型对比、分任务结果、消融分析与特征可视化；5.4 节为本章小结。该模型将与第 4 章的低光增强方法共同构成第 6 章大田验证所用的完整识别流程。本章回答总问题中的联合识别问题：在终端算力约束下，脱叶率与吐絮率能否在一次前向计算中联合获取，精度与计算开销如何权衡。
...
RTCMNet 由共享特征提取主干和两个任务分类头组成。卷积层先提取浅层特征，MSCA Block 再整合多尺度信息，两个分类头分别输出双指标等级。MSCA Block 借鉴 SCTNet@xu2024sctnet 的卷积注意力设计，以卷积计算控制语义建模开销。主干网络提取的特征经自适应平均池化后，分别输入两个结构相同的分类头；每个分类头由全连接层、ReLU@nair2010relu 激活函数和输出层组成，输出相应任务的类别概率。
...
其中 $op("BN")(·)$ 表示 Batch Normalization@ioffe2015batchnorm，$x$、$f$、$y$ 分别表示输入、隐藏特征与输出；$op("MSCA")(·)$ 为多尺度卷积注意力子块，$op("Conv")(·)$ 为逐位置前馈子块（@fig:network (d) 中的 MLP）；两个子块的残差分支均使用 Drop Path 正则化，其思想源于 Dropout@srivastava2014dropout。
...
模型性能采用固定随机种子的五次重复随机划分进行评价。与第 2 章保存的固定划分索引不同，每次都将全部 32 725 个样本按 6:1:3 的比例重新随机划分为训练集、验证集与测试集，五次使用互不相同的随机划分；每次划分分别训练并计算脱叶率与吐絮率任务的准确率、宏平均精确率、宏平均召回率和 Macro-F1，再对两个任务的对应指标取算术平均。最终结果以五次划分双任务平均指标的均值 $plus.minus$ 标准差表示。训练过程中不使用测试集进行最优检查点选择或超参数调整。

本章与第 2 章均以图像块为评价单位；第 2 章保存一份固定的 6:1:3 划分索引，本章则在相同比例下采用五次不同的随机划分，以考察识别结果对随机划分的敏感性。对比模型使用相同的划分索引、输入、训练配置和指标，比较目标作业域内不同网络的性能与部署代价。
...
因此，两项任务的损失权重均为 1。每次划分均使用 Adam 优化器从随机初始化开始训练，不加载 ImageNet@deng2009imagenet 等外部数据集的预训练权重；初始学习率为 $1 times 10^(-3)$，批量大小为 32，不使用权重衰减和学习率调度器。Adam 的一阶与二阶矩估计系数采用 $beta_1 = 0.9$ 和 $beta_2 = 0.999$，数值稳定项为 $epsilon = 10^(-8)$。训练采用 bfloat16 混合精度，以降低显存占用并保持各模型的统一计算精度设置。除骨干结构外，各次划分的数据索引、输入尺寸、分类头、损失函数和优化参数均保持一致，以保证模型比较的公平性。

@fig:training-curves 给出 RTCMNet、ViT-S 与 DenseNet-121 在相同训练协议下的验证损失与双任务验证准确率曲线。正文统一使用 RTCMNet 指代最终采用的配置（1,2,2 层配置与 8 头 MSCA），该配置在训练曲线图中标记为 RTCMNetB1，在分类结果图中标记为 RTCM_b1。DenseNet-121 收敛最快且最平稳，约 500 步后验证损失降至较低水平，脱叶率与吐絮率验证准确率最高分别达到 0.95 和 0.94；ViT-S 的收敛过程也较平稳，但准确率较早进入平台期，分别在 0.8 与 0.7 附近波动；RTCMNet 的验证损失波动更明显，约 550 步处出现一次明显尖峰，双任务验证准确率随训练起伏，但在约 700 步后整体超过 ViT-S，最终轮次的脱叶率与吐絮率验证准确率分别达到 0.83 和 0.89，与参数量约 18 倍于其的 DenseNet-121 末轮水平（0.90 和 0.86）接近。三者的参数量分别为 0.37 M、21.66 M 和 6.63 M：DenseNet-121 以更大的模型规模换取了更快、更稳定的收敛；RTCMNet 在无预训练权重、无学习率调度的设置下训练稳定性相对较弱，属小模型的预期现象，这也是采用五次重复随机划分并以均值 $plus.minus$ 标准差报告结果的原因之一。

#figure(
  image("training_curves.png", width: 100%),
  caption: [相同训练协议下 RTCMNet、ViT-S 与 DenseNet-121 的训练过程对比，横轴为训练步数。（a）验证损失；（b）脱叶率验证准确率；（c）吐絮率验证准确率。],
) <fig:training-curves>
...
上述比较显示，DenseNet-121 的分类指标最高，但移动端时延较长；RTCMNet 以较小模型规模获得接近的分类结果，适合当前设备的快速双指标识别需求。上述时延与精度指标换算到架次级作业节奏后的适用性判断见 6.5 节。
...
较高的宏平均精确率反映了模型在各等级上的整体查准能力。若要据此筛选接近采收条件的区域，还需单独检查高吐絮率等级的误报，并结合现场复核判断。
...
消融结果显示，扩大层数和头数并未带来持续的准确率提升，却明显增加了端侧时延和计算量。RTCMNet 在当前实验中较好地兼顾了准确率与运行效率，适合移动端部署。
...
本章对总问题中联合识别问题的回答是：双指标可在一次前向计算中联合获取，面向任务的轻量设计能够以较小参数量换取接近较大规模模型的识别结果，端侧实测时延满足现场快速识别需求。本章提出正常光照下的轻量级双指标识别模型 RTCMNet，并在统一协议下与经典及轻量网络比较。RTCMNet 以 0.37 M 参数获得接近 DenseNet-121 的双任务分类结果，在 DJI 遥控器上的单张推理耗时为 32 ms。
