...
第 3 章表明，颜色与指数统计优于直接展开的 RGB 像素向量，但这类统计不能完整描述冠层纹理和空间关系。本章设计轻量级双指标识别模型 RTCMNet，以学习多尺度图像特征，并满足端侧推理需求。模型通过对比、分任务评价、消融与实机测试进行验证；4.2—4.4 节依次介绍方法、数据、实验与结果，4.5 节为小结。
...
本章数据为真实棉田的 UAV RGB 影像及人工分级标签，覆盖不同地块与时相。采集、筛选和标注方法见@chap:dataset，本章按下述五折协议比较各模型的识别性能与部署代价。

这组数据有三类识别难点。土壤、滴灌带、残膜、阴影和杂草与棉株共同出现在图像中，局部纹理可能被背景干扰；脱叶与吐絮连续变化，相邻等级缺少清晰的视觉边界；样本数量又集中在部分等级，少数类和临界样本相对不足。因此，模型评价需要同时关注总体准确率与各等级的均衡识别能力。

双指标联合建模利用了两项任务共有的冠层结构、颜色分布、棉絮外露和枝叶遮挡信息。共享骨干可减少特征提取中的重复计算，两个分类头则分别学习脱叶率与吐絮率的等级边界。这一设计旨在利用共同特征，同时保留任务间差异。
...
RTCMNet 为后续低光试验提供正常光照下的识别模型。实际作业中的弱光、逆光和阴影可能改变输入分布，下一章将研究低光数据生成与增强，并检验增强图像接入识别模型后的响应。
