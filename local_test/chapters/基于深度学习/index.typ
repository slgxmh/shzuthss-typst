...
模型性能采用固定随机种子的五次重复随机划分进行评价。与第 2 章保存的固定划分索引不同，每次都将全部样本按 6:1:3 的比例重新随机划分为训练集、验证集与测试集，五次使用互不相同的随机划分；每次划分分别训练并计算脱叶率与吐絮率任务的准确率、宏平均精确率、宏平均召回率和 Macro-F1，再对两个任务的对应指标取算术平均。最终结果以五次划分双任务平均指标的均值 $plus.minus$ 标准差表示。训练过程中不使用测试集进行最优检查点选择或超参数调整。
...
因此，两项任务的损失权重均为 1。每一折均使用 Adam 优化器从随机初始化开始训练，不加载 ImageNet@deng2009imagenet 等外部数据集的预训练权重；初始学习率为 $1 times 10^(-3)$，批量大小为 32，不使用权重衰减和学习率调度器。Adam 的一阶与二阶矩估计系数采用 $beta_1 = 0.9$ 和 $beta_2 = 0.999$，数值稳定项为 $epsilon = 10^(-8)$。训练采用 bfloat16 混合精度，以降低显存占用并保持各模型的统一计算精度设置。除骨干结构外，各折的数据索引、输入尺寸、分类头、损失函数和优化参数均保持一致，以保证模型比较的公平性。
