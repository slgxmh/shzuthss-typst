...
基于上述考虑，本章设计轻量级双指标识别模型 RTCMNet：以多尺度卷积注意力模块增强网络对叶片脱落区域与棉絮区域的特征表达@hu2018senet@woo2018cbam，并以共享主干加分任务输出的结构同时预测脱叶率与吐絮率等级，在控制参数量与计算量的前提下提升识别精度。本章通过骨干网络对比、分任务评价、消融实验和设备推理测试检验模型表现，研究工作的整体流程如@fig:workflow 所示，依次包括无人机影像采集、专家标注与数据预处理、模型训练与比较、端侧推理测试以及真实大田验证。
