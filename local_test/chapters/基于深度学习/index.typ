...
RTCMNet 的特征分布相对均匀，各类别样本在投影空间中形成界限较清晰的簇，与其较高的分类指标呈现一致趋势；部分低性能轻量模型则出现较明显的特征重叠。这一分布形态可以从 MSCA 的结构设计得到机制性解释。棉花脱叶与吐絮的判别线索具有天然的尺度差异：叶缘、棉絮轮廓等属细粒度局部纹理，而冠层稀疏化、白絮成片暴露等属较大范围的空间模式。单尺度卷积的感受野固定，特征响应容易被某一主导尺度的纹理（如大面积土壤背景或成片白絮）支配，使不同等级样本的高层特征被压缩到少数方向上，投影中表现为聚集或重叠。MSCA 以 3×3、5×5 等多尺度可分离卷积分支并行提取不同感受野的响应，并在空间维度上归一化加权，相当于让网络按样本内容自适应地分配对各尺度线索的依赖：细粒度分支保持对叶片残留与棉絮边界的响应，粗尺度分支刻画冠层整体结构，二者经输出映射融合后，高层特征得以同时编码多尺度判别信息，不易坍缩到单一主导模式，投影分布因而更均匀、簇间分离更明显。共享主干再经双分类头分别映射，两项任务得以利用同一套多尺度表征学习各自的等级边界，也与 5.4.6 节消融中 MSCA 优于单尺度 CF 块的结果相互印证。

需要强调的是，t-SNE 通过保留局部邻域结构进行降维，其投影结果受困惑度等超参数影响，簇的形态和间距不直接等价于高维空间中的真实类间距离。因此，该可视化仅用于直观补充不同骨干的特征组织差异，各模块对性能的具体贡献仍以消融实验的定量结果为依据。
