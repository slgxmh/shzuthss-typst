...
第 3 章的对比实验表明，颜色与光谱指数等结构化统计特征显著优于直接展开的 RGB 像素向量，但这类人工统计量不能完整描述冠层的纹理细节与空间结构关系，传统方法的识别精度仍存在上限。深度卷积网络能够直接从数据中学习层次化的图像特征，在农业表型识别任务上已展现出明显优势@lecun2015deep@kamilaris2018deep；然而主流深度模型参数量与计算量大，而棉花脱叶催熟作业的检测需求最终要落到机载或田间端侧设备上，模型必须在精度与推理开销之间取得平衡@howard2017mobilenets@sandler2018mobilenetv2。此外，脱叶率与吐絮率是同一次催熟作业下相伴发生的两个指标，共享冠层颜色与纹理线索，若分别训练两个独立模型，会重复提取相似特征，增加端侧存储与计算开销。

基于上述考虑，本章设计轻量级双指标识别模型 RTCMNet：以多尺度卷积注意力模块增强网络对叶片脱落区域与棉絮区域的特征表达@hu2018senet@woo2018cbam，并以共享主干加分任务输出的结构同时预测脱叶率与吐絮率等级，在控制参数量与计算量的前提下提升识别精度。本章通过骨干网络对比、分任务评价、消融实验和设备推理测试检验模型表现。

本章其余内容安排如下：5.2 节介绍 RTCMNet 的网络结构与多尺度卷积注意力设计；5.3 节说明训练与评价协议，并报告模型对比、分任务结果、消融分析与特征可视化；5.4 节为本章小结。该模型将与第 4 章的低光增强方法共同构成第 6 章大田验证所用的完整识别流程。
...
模型性能采用固定随机种子的五折交叉验证进行评价。全部样本在固定随机种子下随机打乱并划分为五个互斥子集；每轮使用其中一折作为测试集，其余四折作为训练集，直至每一折均作为测试集一次。每一折分别计算脱叶率与吐絮率任务的准确率、宏平均精确率、宏平均召回率和 Macro-F1，再对两个任务的对应指标取算术平均。最终结果以五折双任务平均指标的均值 $plus.minus$ 标准差表示。训练过程中不使用测试折进行最优检查点选择或超参数调整。
...
@fig:training-curves 给出 RTCMNetB1、ViT-S 与 DenseNet-121 在相同训练协议下的验证损失与双任务验证准确率曲线。DenseNet-121 收敛最快且最平稳，约 500 步后验证损失降至较低水平，脱叶率与吐絮率验证准确率最高分别达到 0.95 和 0.94；ViT-S 的收敛过程也较平稳，但准确率较早进入平台期，分别在 0.8 与 0.7 附近波动；RTCMNetB1 的验证损失波动更明显，约 550 步处出现一次明显尖峰，双任务验证准确率随训练起伏，但在约 700 步后整体超过 ViT-S，最终轮次的脱叶率与吐絮率验证准确率分别达到 0.83 和 0.89，与参数量约 18 倍于其的 DenseNet-121 末轮水平（0.90 和 0.86）接近。三者的参数量分别为 0.37 M、21.66 M 和 6.63 M：DenseNet-121 以更大的模型规模换取了更快、更稳定的收敛；RTCMNetB1 在无预训练权重、无学习率调度的设置下训练稳定性相对较弱，属小模型的预期现象，这也是采用五折交叉验证并以均值 $plus.minus$ 标准差报告结果的原因之一。

#figure(
  image("training_curves.png", width: 100%),
  caption: [相同训练协议下 RTCMNetB1、ViT-S 与 DenseNet-121 的训练过程对比，横轴为训练步数。（a）验证损失；（b）脱叶率验证准确率；（c）吐絮率验证准确率。],
) <fig:training-curves>
...
在五折交叉验证中，DenseNet-121 在经典模型组中表现最好（@allmetric）：双任务平均准确率和 Macro-F1 均为 $0.94 plus.minus 0.01$，宏平均精确率为 $0.95 plus.minus 0.01$，宏平均召回率为 $0.94 plus.minus 0.01$。ResNet-18 的准确率为 $0.89 plus.minus 0.04$、宏平均精确率为 $0.91 plus.minus 0.04$；InceptionV3 与 ViT-S 的准确率分别为 $0.79 plus.minus 0.04$ 和 $0.77 plus.minus 0.04$。
...
脱叶率识别的主要难点在于，模型不仅需要识别叶片数量减少这一总体趋势，还需要区分由光照、视角、背景穿透或局部缺株造成的伪稀疏现象。RTCMNet 在该任务上的表现与多尺度特征设计的目标一致，其具体贡献将在消融实验中分析。
...
较高的宏平均精确率反映了模型在各等级上的整体查准能力。若要据此筛选接近采收条件的区域，还需单独检查高吐絮等级的误报，并结合现场复核判断。
...
消融结果显示，扩大层数和头数并未带来持续的准确率提升，却明显增加了端侧时延和计算量。RTCMNetB1 在当前实验中较好地兼顾了准确率与运行效率，适合移动端部署。
...
RTCMNet 的特征分布相对均匀，各类别样本在投影空间中形成界限较清晰的簇，与较高的分类指标相符；部分低性能轻量模型则出现较明显的特征重叠。从结构设计看，叶缘与棉絮轮廓属于细粒度线索，冠层稀疏化与白絮成片暴露则涉及较大范围的空间结构。MSCA 以 3×3、5×5 等多尺度可分离卷积分支提取不同感受野的响应，经归一化和输出映射融合后，为两个分类头提供共同的多尺度特征。这一设计有助于同时利用局部细节与整体冠层信息，前述消融实验也显示其优于单尺度 CF 块。不过，可视化本身不能确定簇间差异由哪一模块造成。

t-SNE 通过保留局部邻域结构进行降维，其投影结果受困惑度等超参数影响，簇的形态和间距不直接等价于高维空间中的真实类间距离。因此，该可视化仅用于直观补充不同骨干的特征组织差异，各模块对性能的具体贡献仍以消融实验的定量结果为依据。
