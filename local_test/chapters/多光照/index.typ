...
第 5 章建立了经真实夜间样本校准的低光成对数据与退化建模流程。本章在此基础上设计轻量低光增强前端 $L^3$-AgriUAVNet，并从图像恢复质量、设备端效率、真实低光表现和 YOLOv8n/YOLOv8s 下游检测响应等方面评价增强模块的适用性。本章其余部分安排如下：6.2 介绍数据、网络架构、损失函数与实验设置；6.3 报告增强质量、运行效率、消融、真实低光和下游检测结果；6.4 讨论农业约束下的模型设计与应用配置；6.5 总结本章工作。
...
逐图像块配对统计支持上述结果。相对 Restormer，$L^3$-AgriUAVNet 的 PSNR 平均提高 0.0878 dB（95% paired bootstrap CI $[0.0470, 0.1301]$，Holm 校正 $p = 0.0105$），SSIM 提高 0.0271（$p < 0.001$）。与 LLFormer 相比，其 PSNR 低 0.2977 dB、SSIM 高 0.00555，MAE 和 LPIPS 则更偏向 LLFormer。$L^3$-AgriUAVNet 在显著缩小模型规模的同时保持了有竞争力的像素保真度，并取得最高结构相似性。
...
Tenengrad 从 33.16 提高至 102.87，边缘密度和局部对比度也同步增加，表明叶片边缘和局部纹理响应得到强化。$L^3$-AgriUAVNet 的 NIQE 为 8.43，与 Raw 的 8.15 接近；LLFormer 和 Retinexformer 则在部分指标上更优。NIQE 与 BRISQUE 的排序并不一致，因此需结合多项无参考指标和@fig:l3_real 的视觉结果评价。$L^3$-AgriUAVNet 在校正欠曝和通道偏差后仍保持了较强的结构响应。
...
YOLOv8s 呈现出不同结果：Raw 输入的 mAP\@0.5 为 0.3574，各增强输入为 0.3328–0.3382。结合 YOLOv8n 的正向响应可见，检测器容量会影响预增强收益。轻量检测器对输入可见度和局部结构预处理更敏感，较大容量检测器则具有更强的特征提取和输入适应能力。因此，增强器需与下游模型联合选择。$L^3$-AgriUAVNet（0.139 M 参数）与 YOLOv8n（约 3.2 M 参数@jocher2023yolov8）的组合仍远小于单独的 YOLOv8s（约 11.2 M 参数），适合无法承担较大检测器的端侧设备。
