...
本章采用合成图像对训练，并以真实低光影像补充评价。合成配对数据来源于在研究区域内邻近采集日期的航线飞行任务中选取的 1306 张正常曝光 UAV 原始影像，覆盖小麦、棉花和玉米的多时相冠层形态、行结构、土壤背景与光照变化。多尺度裁剪、随机旋转和翻转用于生成正常曝光目标图像块并扩展飞行高度、航向和视角变化；随后依据 5.2 节所述流程生成对应的低光输入（见@fig:lowlight_pairgen），构成低光/正常曝光配对样本 $(x, t)$。配对数据以图像块为抽样单位，在固定随机种子 42 下按 0.8/0.1/0.1 划分为训练集、验证集和测试集，固定测试集包含 3265 个图像块。所有对比模型与消融变体使用同一组划分索引，因此 3265 表示由 1306 张源影像构建的测试样本数量，而非额外采集的原始航片数量。由于图像块采样自共享的源影像，图像块级划分可能使同一场景的图像块进入不同子集，因此绝对分数应视为偏乐观的域内估计，而共享的划分索引保证了所有方法间比较的内部公平性。
...
各方法的优势并不集中于同一指标：LLFormer 的 PSNR、MAE 和 LPIPS 更好，SCI 的参数量与 MACs 更低；$L^3$-AgriUAVNet 以 0.139 M 参数获得最高 SSIM，ONNX 文件为 0.79 MiB。这一质量–效率配置的形成并非单一指标优化的结果，其应用价值需要结合结构恢复、存储、计算和设备速度共同判断。
...
这一配置由农业 UAV 的三类约束共同塑造。夜昼影像难以严格对齐，训练依赖可控退化与真实统计校准；幼苗边界和作物行纹理易受平滑与偏色影响，模型需兼顾强度、颜色和局部结构；端侧资源又限制了网络深度与分支数量。$L^3$-AgriUAVNet 因而在同一浅层路径中完成 HVI 表征、局部特征蒸馏和全局曝光调制，减少了双分支结构中的重复特征提取和交互开销：两个 RFDN-S 提取局部纹理，ESA 强化空间结构响应，GMod 根据全局描述子调节曝光与通道比例，各模块分别承担局部、空间和全局信息处理。
...
下游结果显示，预增强的作用与检测器容量相关。在轻量 YOLOv8n 上，紧凑检测器能够利用增强后的可见度与结构响应；RFDN 获得略高的 mAP，但需要约 2.7 倍参数量和 2.8 倍 MACs，进一步体现了增强前端自身预算的重要性。另一方面，LLFormer 的上游 PSNR 最高，但其 YOLOv8n 表现较低，表明像素重建排名不能直接替代任务效用评价。YOLOv8s 在 Raw 输入上的 mAP\@0.5 为 0.3574，各增强输入为 0.3328–0.3382。结合 YOLOv8n 的结果，检测器容量可能影响增强收益，但这一解释仍需重复训练和更多架构验证。$L^3$-AgriUAVNet（0.139 M 参数）与 YOLOv8n（约 3.2 M 参数@jocher2023yolov8）的组合，小于单独的 YOLOv8s（约 11.2 M 参数），可作为资源有限设备的候选配置。

上述结果对田间部署有直接的操作启示：可先确定检测器和预处理预算，再在代表性验证样本上比较 Raw 与增强输入。对当前 YOLOv8n 任务，预算较紧时可考虑 $L^3$-AgriUAVNet；若 RFDN 略高的 mAP 能满足更重要的识别需求，也需同时承担约 2.7 倍参数量和 2.8 倍 MACs。对当前 YOLOv8s 任务，Raw 表现最好，应优先保留原始输入。其他检测器需要单独验证，并使用与部署条件一致的输入分布训练或适配。清晨或傍晚影像是否需要增强，应由任务性能和运行成本决定：若增强改善识别且计算预算允许，可启用轻量前端；若检测器已能较好处理 Raw 输入，跳过增强可节省开销。

== 小结

本章通过 RGB 域退化构建农业 UAV 低光图像对，以解决夜昼样本难以严格配准的问题。退化模型依次模拟曝光衰减、通道偏置、Poisson--Gaussian 噪声、非线性亮度映射、轻度模糊与暗角，并用 26 张真实低光影像校准参数。合成与真实样本的亮度主体区间、通道比例和饱和度分布接近，极暗区和亮度尾部仍有差异。所得图像对用于增强网络的监督训练。
...
需要指出的是，当前退化参数由 26 张真实低光棉花影像的统计分布和视觉观感人工校准，评价范围主要是邻近日期航测所覆盖的生产环境。跨域研究还需补充更多品种、管理条件、传感器、天气和光源下的数据，RAW 或多光谱数据可进一步扩展现有 RGB 域退化模型；任务评价可增加多随机种子重复训练，并扩展到更多检测与分割架构，检验容量相关响应是否普遍成立；设备测试也需进一步覆盖 Jetson 或实际 UAV 计算单元上的端到端时延、峰值内存与能耗。
