...
*与相关架构的关系。*@tab:l3_arch_relation 比较了 $L^3$-AgriUAVNet 的组件来源与结构调整。HVI-CIDNet 提供 HVI 表征，原网络使用强度—色彩双分支和跨注意力；RFDN/RFDB 提供逐级特征蒸馏与 ESA，原结构包含较宽的细化路径、多块输出融合和恢复头；SCI 则采用更轻量的照明估计。本文将 HVI 与特征蒸馏组织为单流主干，使用两个浅层块、一次自条件全局调制和 HVI 残差重建，以适应农业无人机的参数预算。
...
本章采用合成图像对训练，并以真实低光影像补充评价。合成配对数据来源于在研究区域内邻近采集日期的航线飞行任务中选取的 1306 张正常曝光无人机原始影像，覆盖小麦、棉花和玉米的多时相冠层形态、行结构、土壤背景与光照变化。多尺度裁剪、随机旋转和翻转用于生成正常曝光目标图像块并扩展飞行高度、航向和视角变化；随后依据 4.2 节所述流程生成对应的低光输入（见@fig:lowlight_pairgen），构成低光/正常曝光配对样本 $(x, t)$。配对数据以图像块为抽样单位，在固定随机种子 42 下按 0.8/0.1/0.1 划分为训练集、验证集和测试集，固定测试集包含 3265 个图像块。所有对比模型与消融变体使用同一组划分索引，因此 3265 表示由 1306 张源影像构建的测试样本数量，而非额外采集的原始航片数量。由于图像块采样自共享的源影像，图像块级划分可能使同一场景的图像块进入不同子集，因此绝对分数应视为偏乐观的域内估计，而共享的划分索引保证了所有方法间比较的内部公平性。

连续航测影像保留了相邻田间区域的结构相似性。统一图像块划分便于在相同输入条件下比较模型，但共享源影像带来的相关性仍然存在，不能将方法比较的内部一致性等同于测试场景的独立性。

退化参数标定采用另外采集的 26 张真实低光棉花无人机原始影像，依据亮度、饱和度、RGB 通道比例分布和视觉观感人工调整采样范围。这 26 张影像仅用于标定，不参与模型训练及后续玉米苗评价。@fig:lowlight_stats 显示，合成样本覆盖了真实低光影像的暗强度与通道比例区间，表明校准参数能够刻画当前农业场景的主要低光统计特征。

真实场景评价采用玉米苗低光无人机影像，与 26 张棉花标定影像相互独立。308 个图像块缺少严格配对的正常曝光参考，用于视觉比较和无参考质量评价；幼苗中心点标注则转换为检测框，用于训练检测器，并在固定的 139 个测试图像块上评价检测性能。两类评价分别考察图像外观变化和任务效果。
...
下游玉米苗检测对两种检测器配置均报告 mAP\@0.5、精确率和召回率；YOLOv8n 另报告 COCO 风格的 mAP\@0.5:0.95。
...
@tab:l3_overall 中，质量指标为固定划分的 3265 个测试图像块上逐块测量的算术均值，PSNR/SSIM 越高、MAE/LPIPS 越低越好；参数量为模型总参数量，MACs 使用 THOP 在 evaluation mode、batch size 为 1、$1 times 3 times 224 times 224$ 输入下统计；各数值列最优值以粗体标出，设备端延迟见@tab:l3_ipad，$L^3$-AgriUAVNet 指标准卷积配置。表中显示，$L^3$-AgriUAVNet 以 0.139 M 参数取得 26.24 dB PSNR 和 0.893 SSIM，其中 SSIM 为所有方法最高。其 PSNR 比 Restormer 高 0.09 dB，距 LLFormer 仅 0.30 dB。统一计算量统计条件下，模型为 6.252 GMACs，ONNX 文件大小为 0.79 MiB；相比之下，LLFormer、Retinexformer 和 RFDN 的模型大小分别为 22.62、8.30 和 1.45 MiB。
...
逐图像块配对分析进一步量化了模型间的差异。相对 Restormer，$L^3$-AgriUAVNet 的 PSNR 平均提高 0.0878 dB（95% paired bootstrap CI $[0.0470, 0.1301]$，Holm 校正 $p = 0.0105$），SSIM 提高 0.0271（$p < 0.001$）。相对 LLFormer，其 PSNR 低 0.2977 dB、SSIM 高 0.00555，MAE 和 LPIPS 则由 LLFormer 占优。因此，在当前测试集上，$L^3$-AgriUAVNet 的主要特点是较小规模和较高结构相似性，像素误差与感知距离仍有改进空间。

各方法的优势并不集中于同一指标：LLFormer 的 PSNR、MAE 和 LPIPS 更好，SCI 的参数量与 MACs 更低；$L^3$-AgriUAVNet 以 0.139 M 参数获得最高 SSIM，ONNX 文件为 0.79 MiB。选择增强模型时，应结合结构恢复质量、模型存储和设备速度确定合适配置。
...
@tab:l3_ipad 的测试协议为：所有方法均使用 ONNX Runtime Web 1.23.2、WebGPU execution provider，启用图优化，输入为随机 float32 $1 times 3 times 224 times 224$，batch size 为 1；5 次 warm-up 后计时 10 次并取平均，计时不含会话创建、模型加载、预处理和后处理。在该固定协议下，$L^3$-AgriUAVNet 相对 LLFormer、RFDN 和 Retinexformer 分别约快 2.8、6.3 和 13.4 倍。该结果直接验证了模型在 iPad Air（第 3 代；Apple Inc., Cupertino, CA, USA）、$224 times 224$ 输入和 ONNX Runtime Web WebGPU 后端上的软件端推理效率，说明该模型具备集成到农业无人机图像处理流程中的速度条件。
...
这一配置由农业无人机的三类约束共同塑造。夜昼影像难以严格对齐，训练依赖可控退化与真实统计校准；幼苗边界和作物行纹理易受平滑与偏色影响，模型需兼顾强度、颜色和局部结构；端侧资源又限制了网络深度与分支数量。$L^3$-AgriUAVNet 因而在同一浅层路径中完成 HVI 表征、局部特征蒸馏和全局曝光调制，减少了双分支结构中的重复特征提取和交互开销：两个 RFDN-S 提取局部纹理，ESA 强化空间结构响应，GMod 根据全局描述子调节曝光与通道比例，各模块分别承担局部、空间和全局信息处理。
...
@tab:l3_ablation 通过一次移除或替换一个组件来评估各架构贡献，“std”表示标准卷积，“DW”表示深度可分离卷积，$checkmark$ 表示启用，“--”表示禁用。表中显示，全局仿射调制对当前结构贡献最大：移除 GMod 后，PSNR 下降约 2.62 dB。移除 HVI 表征和 ESA 分别带来约 1.73 和 0.65 dB 的下降，说明全局曝光调节、强度–色彩组织与局部空间响应在浅层主干中发挥互补作用。将标准卷积替换为深度可分离卷积后，PSNR 下降 0.54 dB，表明通道间充分混合对于该低深度网络仍然重要。
...
@fig:l3_real 依次展示 Raw、RFDN、Retinexformer、LLFormer 和 $L^3$-AgriUAVNet 的增强结果，定性观察与无参考统计从互补维度刻画了增强效果；该数据与用于退化参数标定的 26 张棉花低光图像相互独立。@tab:l3_real_no_ref 中，Raw 与各增强输出按文件名一一对齐（$n = 308$ 个图像块），各数值为逐图计算后的算术均值；NIQE、BRISQUE、欠曝像素比例和 RGB 通道均衡偏差设有明确优化方向，Tenengrad、边缘密度和局部对比度作为描述性结构响应，不进行单调优劣排序；表中的 $L^3$-AgriUAVNet 指标准卷积配置。$L^3$-AgriUAVNet 将平均欠曝像素比例从 23.74% 降至近 0，RGB 通道均衡偏差由 0.0346 降至 0.0233（降低约 32.5%），BRISQUE 也从 52.21 降至 48.58。阈值化的欠曝像素比例在所有增强输出上均接近 0，因此该指标用于验证欠曝的消除，而非对增强方法排序。RFDN 的 RGB 通道均衡偏差反而升至 0.0611，说明可见度改善并不必然伴随全局通道关系的校正。
...
#figure(
  three-line-table(table(
    columns: 5,
    table.header([输入], [mAP\@0.5], [mAP\@0.5:0.95], [精确率], [召回率]),
...
*YOLOv8s：高容量检测器。*检测器使用与 YOLOv8n 相同的数据划分和训练安排，分别在 Raw 输入和各增强方法的输出分布上独立训练；测试集包含 139 个图像块，表中数值为单次运行的点估计。@tab:l3_yolov8s 显示，Raw 输入的 mAP\@0.5 为 0.3574，$L^3$-AgriUAVNet、RFDN、LLFormer 和 Retinexformer 分别为 0.3338、0.3328、0.3370 和 0.3382，呈现出与 YOLOv8n 不同的响应模式。检测器容量及其特征提取能力可能改变预增强的边际收益，这一关系将在重复训练和更多检测架构中继续验证。

下游结果显示，预增强的作用与检测器容量相关。在轻量 YOLOv8n 上，紧凑检测器能够利用增强后的可见度与结构响应；RFDN 获得略高的 mAP，但需要约 2.7 倍参数量和 2.8 倍 MACs，说明增强前端的资源需求也应纳入比较。另一方面，LLFormer 的上游 PSNR 最高，但其 YOLOv8n 表现较低，表明像素重建排名不能直接替代任务效用评价。YOLOv8s 在 Raw 输入上的 mAP\@0.5 为 0.3574，各增强输入为 0.3328–0.3382。结合 YOLOv8n 的结果，检测器容量可能影响增强收益，但这一解释仍需重复训练和更多架构验证。$L^3$-AgriUAVNet（0.139 M 参数）与 YOLOv8n（约 3.2 M 参数@jocher2023yolov8）的组合，小于单独的 YOLOv8s（约 11.2 M 参数），可作为资源有限设备的候选配置。

田间部署可据此选择处理方式：可先确定检测器和预处理预算，再在代表性验证样本上比较 Raw 与增强输入。对当前 YOLOv8n 任务，预算较紧时可考虑 $L^3$-AgriUAVNet；若 RFDN 略高的 mAP 能满足更重要的识别需求，也需同时承担约 2.7 倍参数量和 2.8 倍 MACs。对当前 YOLOv8s 任务，Raw 表现最好，应优先保留原始输入。其他检测器需要单独验证，并使用与部署条件一致的输入分布训练或适配。清晨或傍晚影像是否需要增强，应由任务性能和运行成本决定：若增强改善识别且计算预算允许，可启用轻量前端；若检测器已能较好处理 Raw 输入，跳过增强可节省开销。

== 小结

本章对总问题中低光恢复问题的回答是：农业低光退化可由曝光衰减、通道偏移、噪声、模糊与空间照度变化等可控算子在 RGB 域近似建模，轻量增强网络能够恢复冠层结构与颜色信息；但增强对下游识别的收益与检测器配置相关，并非在所有条件下都有益。本章通过 RGB 域退化构建农业无人机低光图像对，以解决夜昼样本难以严格配准的问题。退化模型依次模拟曝光衰减、通道偏置、Poisson--Gaussian 噪声、非线性亮度映射、轻度模糊与暗角，并用 26 张真实低光影像校准参数。合成与真实样本的亮度主体区间、通道比例和饱和度分布接近，极暗区和亮度尾部仍有差异。所得图像对用于增强网络的监督训练。

以校准退化数据训练的单流 HVI 低光增强网络 $L^3$-AgriUAVNet 含 0.139 M 参数，在合成配对测试集上取得 26.24 dB PSNR 和 0.893 SSIM；输入为 $1 times 3 times 224 times 224$ 时，计算量为 6.252 GMACs。与对比方法相比，其特点是以较小模型规模保持较高结构相似性。

消融与逐图像块配对分析表明，GMod、HVI 和 ESA 分别贡献约 2.62、1.73 和 0.65 dB 的 PSNR 增益，标准卷积在当前浅层配置中比 DWConv 基线高 0.54 dB。独立于 26 张棉花低光标定图像的 308 个真实低光玉米苗图像块进一步验证了模型的场景适应性：欠曝像素比例由 23.74% 降至近 0，RGB 通道均衡偏差由 0.0346 降至 0.0233。在具有检测真值的 139 个测试图像块上，增强输入使 YOLOv8n 的 mAP\@0.5 相对 Raw 提高 14.6%；但 YOLOv8s 的 Raw 输入达到 0.3574，高于各增强输入的 0.3328--0.3382。上述检测结果均为单次运行点估计，表明增强效果与检测器配置有关。一种解释是，低光退化首先损害颜色与通道间关系，轻量检测器难以在有限容量下自行补偿，增强恢复的颜色信息因而可直接转化为检测收益；高容量检测器可在特征提取中部分适应输入退化，增强带来的外观改变反而可能偏离其训练分布。$L^3$-AgriUAVNet 可作为资源受限农业无人机视觉管线的候选低光前端，是否启用仍需依据下游模型和目标输入分布验证。

当前退化参数由 26 张真实低光棉花影像的统计分布和视觉观感人工校准，评价范围主要是邻近日期航测所覆盖的生产环境。跨域研究还需补充更多品种、管理条件、传感器、天气和光源下的数据，RAW 或多光谱数据可进一步扩展现有 RGB 域退化模型；任务评价可增加多随机种子重复训练，并扩展到更多检测与分割架构，检验容量相关响应是否普遍成立；设备测试也需进一步覆盖 Jetson 或实际无人机计算单元上的端到端时延、峰值内存与能耗。
