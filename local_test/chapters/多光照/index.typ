...
本章采用合成图像对训练，并以真实低光影像补充评价。合成配对数据来源于在研究区域内邻近采集日期的航线飞行任务中选取的 1306 张正常曝光无人机原始影像，覆盖小麦、棉花和玉米的多时相冠层形态、行结构、土壤背景与光照变化。多尺度裁剪、随机旋转和翻转用于生成正常曝光目标图像块并扩展飞行高度、航向和视角变化；随后依据 5.2 节所述流程生成对应的低光输入（见@fig:lowlight_pairgen），构成低光/正常曝光配对样本 $(x, t)$。配对数据以图像块为抽样单位，在固定随机种子 42 下按 0.8/0.1/0.1 划分为训练集、验证集和测试集，固定测试集包含 3265 个图像块。所有对比模型与消融变体使用同一组划分索引，因此 3265 表示由 1306 张源影像构建的测试样本数量，而非额外采集的原始航片数量。由于图像块采样自共享的源影像，图像块级划分可能使同一场景的图像块进入不同子集，因此绝对分数应视为偏乐观的域内估计，而共享的划分索引保证了所有方法间比较的内部公平性。
...
本章通过 RGB 域退化构建农业无人机低光图像对，以解决夜昼样本难以严格配准的问题。退化模型依次模拟曝光衰减、通道偏置、Poisson--Gaussian 噪声、非线性亮度映射、轻度模糊与暗角，并用 26 张真实低光影像校准参数。合成与真实样本的亮度主体区间、通道比例和饱和度分布接近，极暗区和亮度尾部仍有差异。所得图像对用于增强网络的监督训练。
