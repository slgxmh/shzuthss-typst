...
本研究围绕成熟期棉花脱叶率与吐絮率的快速、准确与可部署检测需求，针对真实大田场景中背景复杂、光照多变、算力受限和应用验证不足等约束，构建了涵盖数据集建立、可解释基线建模、端侧轻量深度学习、低光与多光照鲁棒性增强以及大田作业实验的完整研究链路。全文工作旨在将成熟期棉花双指标状态由经验判断对象转化为可计算、可验证并可服务田间作业的遥感表型信息。结合全文研究内容，可得到以下结论。
...
（2）基于光谱指数、形态和纹理的结构化特征链路能够为双指标识别提供可解释基线，但其性能受背景复杂性与类别不均衡制约。第 3 章构建了“前景分割—结构化特征—传统机器学习”的基线方法，并通过 Macro-F1 等指标分析了不同模型在双指标任务中的表现。实验结果表明，PCA+Logistic Regression 在当前设置下优于 Random Forest 系列，说明成熟期棉田图像中的部分有效信息可通过低维结构化特征刻画；同时，Accuracy 与 Macro-F1 的明显差异也表明传统模型在长尾等级场景下容易偏向多数类。该基线为后续深度模型的性能分析提供了可解释参照。
...
（4）全天候识别需要围绕“退化建模—增强前端—下游识别”建立协同的数据与方法体系。第 5 章构建了面向农业 UAV 夜间场景的低光成对数据集，通过物理与 ISP 启发的退化模型生成可控监督样本，并利用真实夜间图像进行统计一致性校验；第 6 章进一步提出增强前端与识别模型协同工作的多光照鲁棒识别链路。实验结果表明，所提轻量增强前端 $L^3$-AgriUAVNet 参数量仅为 0.139 M，SSIM 达 0.893 并在跨复杂度全谱系比较中为全表最优，LPIPS 为 0.119、与最优方法（LLFormer 0.103）处于同一水平；在 iPad 级平板设备上单张推理耗时 202 ms，可满足端侧近实时增强需求。增强前端能够改善亮度分布、结构恢复与颜色稳定性，从而为后续双指标识别提供更稳定的输入，并降低复杂光照条件下的性能波动。
...
综合上述五点可以看出，本研究围绕成熟期棉花催熟监测这一具体农业问题，建立了由数据、方法到田间验证的完整研究路径。该路径在保留可解释基线支撑作用的同时，通过轻量深度模型和多光照鲁棒设计提高了复杂场景下的识别能力，并在大田作业实验中完成了应用层验证。
